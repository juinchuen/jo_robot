...
leg_position[:,0] = 30 * np.cos(t) + 10
leg_position[:,1] = sway * np.sin(t)
leg_position[:,2] = np.maximum(-rest_height + 30 * np.sin(t), -rest_height)

angles1[:,:5] = ik.points2angles(leg_position, False)

leg_position[:,0] = 30 * np.cos(t + np.pi) + 10
leg_position[:,1] = sway * np.sin(t)
leg_position[:,2] = np.maximum(-rest_height + 30 * np.sin(t + np.pi), -rest_height)

...
if __name__ == '__main__':
    ...
    step_pos = np.array([[0*stride, 0.00],
                         [1*stride,  hip],
                         [2*stride, -hip],
                         [3*stride,  hip],
                         [4*stride, -hip],
    ...
    t_preview = 2.0 # timing for preview
    n_step = len(step_pos)
    ...
    #plt.plot(-0.025*np.sin(np.linspace(0,9*3.1415,450)), 'k--', label='sine')
    #plt.legend()
    ...
    #plt.figure()
    #plt.plot(ZMP_x_ref[200:300], 'g--', label='ZMP_x_ref')
    #plt.plot(ZMP_x_record_1[200:300], 'b', label='ZMP_x')
    #plt.plot(COM_x_record_1[200:300], 'r--', label='COM_x')
    #plt.legend()

    t1 = np.linspace(0,1,100)
    t2 = np.linspace(0,1,16)
    z = zmp(64, 70, 180)
    ZMP_ideal = z.gen_footsteps(t_step, stride*1000, 40, 1, 16)/1000
    plt.figure()
    plt.plot(t1, ZMP_x_ref[200:300], 'g--', label='ZMP_x_ref')
    plt.plot(t1, ZMP_x_record_1[200:300], 'b', label='ZMP_x')
    plt.plot(t1, COM_x_record_1[200:300], 'r--', label='COM_x')
    plt.plot(t2, ZMP_ideal[:16,0] + 0.1, 'k--', label='ZMP_ideal')
    plt.legend()
